...
import { defer, mergeMap, Observable, of, share, shareReplay, switchMap, tap } from 'rxjs'
...
export class WebSocketServer {
  ...
  constructor(
    ...
    this.server$ = defer(() => of(new Server({ ...config, server: httpServer }))).pipe(
      switchMap(server => new Observable<Server>(o => {
    ...
    this.connection$ = this.listening$.pipe(
      switchMap(server => new Observable<WebSocketConnection>(o => {
    ...
    this.headers$ = this.listening$.pipe(
      switchMap(server => new Observable<WebSocketHeaders>(o => {
