...
export function defineScope<TDescription extends string>(description: TDescription): PredefinedInjectionScope<typeof description> {
  const key = Symbol.for(description)
  if (DEFINED_SCOPE_KEYS.has(key)) {
    throw new Error(`The application has already defined a scope using the key '${description}'. Use a unique key for each predefined scope.`)
  }
