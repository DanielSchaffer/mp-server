...
export class DebugDisplay {
  ...
  public bind(avatar$: Observable<Avatar>): Observable<never> {
    return avatar$.pipe(
      tap(avatar => {
        this.updateComponent(this.velocity, `${Point.absTotal(avatar.movement.velocity).toFixed(2)}m/s`)
        this.updateComponent(this.acceleration, `${Point.absTotal(avatar.movement.acceleration).toFixed(2)}m/s/s`)
        this.updateComponent(this.orientation, `${avatar.position.orientation.y.toFixed(2)}º`)
        this.updateComponent(this.rotation, `${avatar.rotation.rate.y.toFixed(2)}º/s`)
        this.updateComponent(this.location, avatar.position.location)
      }),
      silence(),
    )
  }
